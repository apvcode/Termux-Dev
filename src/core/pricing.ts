...
export function getModelPricing(modelName: string): { input: number; output: number } {
  ...
  if (clean.includes('gpt-4o')) return { input: 2.5, output: 10.0 };
  if (clean.includes('deepseek')) return { input: 0.14, output: 0.28 };
  if (clean.includes('claude-3-5-sonnet') || clean.includes('claude-3.5-sonnet')) return { input: 3.0, output: 15.0 };
  if (clean.includes('gemini-2.0-flash') || clean.includes('gemini-1.5-flash')) return { input: 0.1, output: 0.4 };
