...
export class RepoMapGenerator {
   ...
   */
  static async generate(rootDir: string = process.cwd(), maxFiles: number = 60): Promise<string> {
    try {
      const files = await this.scanFiles(rootDir, maxFiles);
      if (files.length === 0) return '';

      const lines: string[] = ['Project Structure & Key Symbols (Repo Map):'];
  ...
  private static async scanFiles(dir: string, maxFiles: number): Promise<FileSummary[]> {
  ...
  private static async extractSymbols(filePath: string, ext: string): Promise<string[]> {
    const symbols: string[] = [];
    try {
      // Read at most 16KB of each file for super-fast symbol extraction
      const stat = await fs.stat(filePath);
      if (stat.size > 256 * 1024) return []; // Skip giant generated files
      ...
      for (const line of lines) {
        ...
        if (ext === '.ts' || ext === '.tsx' || ext === '.js' || ext === '.jsx' || ext === '.mjs') {
          ...
          if (fnMatch) { symbols.push(`fn ${fnMatch[1]}`); continue; }

          const classMatch = trimmed.match(/^export\s+(?:abstract\s+)?class\s+([a-zA-Z0-9_$]+)/);
          if (classMatch) { symbols.push(`class ${classMatch[1]}`); continue; }

          const ifaceMatch = trimmed.match(/^export\s+interface\s+([a-zA-Z0-9_$]+)/);
          if (ifaceMatch) { symbols.push(`interface ${ifaceMatch[1]}`); continue; }

          const typeMatch = trimmed.match(/^export\s+type\s+([a-zA-Z0-9_$]+)/);
          if (typeMatch) { symbols.push(`type ${typeMatch[1]}`); continue; }

          const constMatch = trimmed.match(/^export\s+const\s+([a-zA-Z0-9_$]+)/);
          if (constMatch && !constMatch[1].startsWith('_')) { symbols.push(constMatch[1]); continue; }
        } else if (ext === '.py') {
          // Python classes and functions
          const pyClass = trimmed.match(/^class\s+([a-zA-Z0-9_]+)/);
          if (pyClass) { symbols.push(`class ${pyClass[1]}`); continue; }

          const pyFn = trimmed.match(/^(?:async\s+)?def\s+([a-zA-Z0-9_]+)/);
          if (pyFn && !pyFn[1].startsWith('__')) { symbols.push(`def ${pyFn[1]}`); continue; }
        } else if (ext === '.rs') {
          const rsFn = trimmed.match(/^pub\s+(?:async\s+)?fn\s+([a-zA-Z0-9_]+)/);
          if (rsFn) { symbols.push(`fn ${rsFn[1]}`); continue; }
          const rsStruct = trimmed.match(/^pub\s+struct\s+([a-zA-Z0-9_]+)/);
          if (rsStruct) { symbols.push(`struct ${rsStruct[1]}`); continue; }
        } else if (ext === '.go') {
          const goFn = trimmed.match(/^func\s+(?:\([^\)]+\)\s+)?([a-zA-Z0-9_]+)/);
          if (goFn) { symbols.push(`func ${goFn[1]}`); continue; }
        }
      }
    } catch {}
