...
export interface CustomCommand {
  ...
  cmd: string;  // e.g. "/deploy"
  desc: string; // description from frontmatter or summary
...
export class CustomCommandManager {
   ...
   */
  static async listCommands(forceRefresh = false): Promise<CustomCommand[]> {
    const now = Date.now();
   ...
   */
  static async findCommand(cmdName: string): Promise<CustomCommand | null> {
   ...
   */
  static expandTemplate(template: string, args: string): string {
   ...
   */
  private static parseCommandFile(rawContent: string, defaultName: string): { description: string; template: string } {
    let description = '';
