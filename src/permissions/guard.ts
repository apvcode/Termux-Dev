 ...
 */
function isPathOutsideCwd(targetPath?: string): boolean {
 ...
 */
function isDangerousBashCommand(commandStr?: string): boolean {
  ...
  const isDangerousRm =
    /rm\s+-(?:[a-z]*r[a-z]*f|[a-z]*f[a-z]*r)\s+(?:\/|\/\*|~|~\/|\*|\.|\.\/|\.\.\/?.*|\$home|\$prefix|\/data\/data\/com\.termux)/i.test(norm) ||
 ...
 */
function hasComplexChainingOrRedirection(commandStr?: string): boolean {
  if (!commandStr) return false;
  return (
    commandStr.includes('&&') ||
    commandStr.includes('||') ||
...
export class CLIConsoleGuard implements PermissionGuard {
  ...
  async askUser(toolName: string, args: any): Promise<boolean> {
    ...
    let allowed: boolean | symbol = false;
    try {
