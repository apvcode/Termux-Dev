...
export class GitStatusCache {
  private static cachedInfo: GitStatusInfo | null | undefined = undefined;
  private static lastCheckTime = 0;
  private static readonly TTL_MS = 10000; // 10s fallback TTL

  public static getStatus(forceRefresh = false): GitStatusInfo | null {
    const now = Date.now();
...
export function renderProgressBar(percent: number, barWidth = 10): string {
  const clamped = Math.max(0, Math.min(100, percent));
  const filledCount = Math.round((clamped / 100) * barWidth);
  const emptyCount = Math.max(0, barWidth - filledCount);
  const bar = '█'.repeat(filledCount) + '░'.repeat(emptyCount);

  if (clamped >= 90) return pc.red(`[${bar}] ${clamped}%`);
  if (clamped >= 75) return pc.yellow(`[${bar}] ${clamped}%`);
  return pc.cyan(`[${bar}] ${clamped}%`);
}

export function formatCostBadge(cost: number, maxCostUSD?: number): string {
...
export function renderPowerlineStatus(opts: PowerlineOptions): string {
