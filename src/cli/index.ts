  ...
  while (true) {
    try {
      ...
      const choice = await select({
        message: `${pc.bold('⚙️  Settings')} ${pc.dim(`(devx v1.4.2 • theme: ${currentTh.name})`)}`,
        choices: [
          {
          ...
          {
            name: `${currentTh.colorFn('✨ About devx')} ${pc.dim('(v1.4.2 by ApvCode)')}`,
            value: 'about',
      ...
      if (choice === 'about') {
        p.note(
          `⚡ devx v1.4.2 — Terminal-Native AI Coding Agent\n` +
